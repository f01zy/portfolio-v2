import styles from "@/components/Home/Projects/Project/Project.module.scss"
...
const Project: FC<IProject> = ({ description, name, status }) => {
  return <div className={styles.slide}>
    <div className={styles.slide_info}>
      ...
      <h2>{name}</h2>
      <p className={styles.pg}>{description}</p>
      <button>Visit app website</button>
    </div>
    <div className={styles.slide_image}>
      <Image src={`/projects/${name.toLowerCase()}.png`} alt="project" width={830} height={464} />
    </div>
  </div>
}
